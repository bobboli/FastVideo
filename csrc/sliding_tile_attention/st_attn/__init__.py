...
from st_attn_cuda import sta_fwd_688
...
def sliding_tile_attention(q_all, k_all, v_all, window_size, text_length, has_text=True):
    ...
    #assert q_all.shape[2] == 30 * 80 * 48 + 540, "STA currently only supports video with latent size (16, 32, 56) with 256 text tokens"
    ...
    # assert q_all.shape[2] == (5*6) * (6*8) * (10*8) + 540, "STA currently only supports video with latent size (30, 48, 80) with 540 text tokens, but got " + str(q_all.shape[2])
    assert q_all.shape[1] == len(window_size), "Number of heads must match the number of window sizes"

...
def flex_test(Q, K, V, kernel_size):
    mask = get_sliding_tile_attention_mask(kernel_size, (6, 8, 8), (30, 48, 80), 256, 'cuda', 0)
...
def h100_fwd_kernel_test(Q, K, V, kernel_size):
    o = sliding_tile_attention(Q, K, V, [kernel_size] * 24, 256, True)
    return o
...
def generate_error_graphs(b, h, d, causal, mean, std, error_mode='all'):
    seq_lengths = [30 * 48 * 80 + 256] # [82944]

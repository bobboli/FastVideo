...
from st_attn import sliding_tile_attention_templatized
from torch.nn.attention.flex_attention import flex_attention
from tqdm import tqdm

print("Testing sliding_tile_attention_templatized implementation with text processing enabled")
...
def flex_test(Q, K, V, kernel_size):
    ...
    mask = get_sliding_tile_attention_mask(kernel_size, tile_size, img_size, text_length, 'cuda', text_max_len)
    output = flex_attention(Q, K, V, block_mask=mask)
    return output


def h100_fwd_kernel_test(Q, K, V, kernel_size):
    # Create a list of kernel_size repeated h times for all heads
    window_size = [kernel_size] * h
    print(kernel_size)
    
    # Use the templatized version with all required parameters
    o = sliding_tile_attention_templatized(
        Q, K, V, 
        img_size=img_size,
        tile_size=tile_size,
    ...
    cuda_error = torch.cuda.synchronize()
    if cuda_error:
        raise RuntimeError(f"CUDA error occurred: {cuda_error}")
    return o
...
def generate_error_graphs(b, h, d, causal, mean, std, error_mode='all'):
    seq_lengths = [img_size[0] * img_size[1] * img_size[2] + text_max_len]
...
print("=" * 50)
print("TESTING TEMPLATIZED IMPLEMENTATION WITH TEXT PROCESSING")
print("=" * 50)
...
print("Text processing tests completed for templatized implementation.")
